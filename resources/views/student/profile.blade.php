...
@section('content')
<div class="col-10 col-sm-8 col-md-6 col-xl-5 p-5 rounded text-center shadow mb-4 bg-light">
    <h4 class="mb-4">
        @auth
        ...
        <div class="input-group form-group mt-4 mb-0">
            ...
            <select name="course" id="course" class="form-control">
                ...
                @foreach ($courses as $course )
                    <option value="{{ $course->id }}" @php if(auth()->user()->student) echo (auth()->user()->student->course_id == $course->id)? "selected" : "" @endphp>{{ $course->course_code }} : {{ $course->course_desc }} @if($course->major) - Major in {{ $course->major }} @endif</option>
                @endforeach
            </select>
        </div>  
            @error('course')
