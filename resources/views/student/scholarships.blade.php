...
@section('active_view')
        <h4 class="mb-4">
            {{ ($read_sch)? 'Update Scholarship Application' : 'Apply for Scholarship' }}
        </h4 >  
        @if (session('status'))
        ...
        @csrf
            ...
            <div class="input-group form-group mt-4 mb-0">
                ...
                <input type="text"  name="school_year" id="school_year" placeholder="School Year (e.g. 2020-2021)" class="form-control" value="{{ ($read_sch)? ($read_sch->sy) : old('school_year') }}">
            </div>
            @error('school_year')
                <p class="text-danger text-left mb-0 pb-0"><small> {{ $message }} </small></p>
            @enderror
            ...
            <div class="input-group form-group mt-4 mb-0">
                ...
                <input type="number" step="0.01" min="1.00" max="5.00" name="gpa" id="gpa" placeholder="Enter Your GPA (e.g. 1.27)" class="form-control" @if($read_sch) value="{{$read_sch->gpa}}" @else  value="{{ old('gpa') }}" @endif>
            </div>
            @error('gpa')
                <p class="text-danger text-left mb-0 pb-0"><small> {{ $message }} </small></p>
            @enderror
            ...
            <div class="input-group form-group mt-4 mb-0">
                ...
                <input type="number" step="0.01" min="1.00" max="5.00" name="lowest_grade" id="lowest_grade" placeholder="Lowest Subject Grade (e.g 2.00)" class="form-control" @if($read_sch) value="{{$read_sch->lowest_grade}}" @else  value="{{ old('lowest_grade') }}" @endif }}">
            </div>
            @error('lowest_grade')
                <p class="text-danger text-left mb-0 pb-0"><small> {{ $message }} </small></p>
            @enderror
            ...
            <div class="input-group form-group mt-4 @error('num_of_units') mb-0 @else mb-4 @endif">
                ...
                <input type="number" step="1" min="0" name="num_of_units" id="num_of_units" placeholder="Number of Units" class="form-control" @if($read_sch) value="{{$read_sch->num_of_units}}" @else  value="{{ old('num_of_units') }}" @endif>
            </div>
            @error('num_of_units')
            ...
            <div class="form-group form-check-inline">
                <input name="has_inc" type="checkbox" class="form-check-input" id="has_inc" @if($read_sch) {{ ($read_sch->has_inc)? 'checked': '' }} @else {{ (old('has_inc'))? 'checked' : '' }} @endif>
            ...
            @enderror
            <div class="form-group form-check-inline">
                <input name="has_drop" type="checkbox" class="form-check-input" id="has_drop" @if($read_sch) {{ ($read_sch->has_drop)? 'checked': '' }} @else {{ (old('has_drop'))? 'checked' : '' }} @endif>
            ...
            @if($read_sch) 
                ...
                <a href="{{ route('student.scholarships') }}" class="btn btn-danger btn-block mt-4">Cancel Edit</a>  
            @else 
